...
// ══ REVIEWS ════════════════════════════════════════════════════════════════════
const REVIEW_STORAGE_KEY = 'beoxide_reviews';
...
function cleanReviews(data) {
  ...
  return data.filter(entry => {
    ...
    if (lowerName.includes('kushal')) return false;
    return true;
  });
}
...
function loadReviews() {
  try {
    ...
    if (cleaned.length !== raw.length) {
      localStorage.setItem(REVIEW_STORAGE_KEY, JSON.stringify(cleaned));
    }
    return cleaned;
  } catch {
    return [];
...
function buildReviews() {
  ...
  if (!REVIEWS.length) {
    row.innerHTML = '<div class="empty">No reviews yet. Be the first to share your experience.</div>';
    return;
  }
  ...
  row.innerHTML = REVIEWS.map(r => `
    <div class="review-card">
      <div class="review-card-header">
        ...
        <div class="review-product">${r.product || 'OneTon Collection'}</div>
      </div>
...
function initApp() {
  ...
  if (reviewForm) {
    reviewForm.addEventListener('submit', e => {
      ...
      addReview({
        name: (formData.name || 'Guest').trim() || 'Guest',
        stars: Number(formData.stars) || 5,
        text: reviewText,
        product: 'OneTon Collection',
